...
%:- use_module(library('knowrob_map_tools')).
...
detectObject(Obj, Material_str, Shape_str, Affordance_str):-
	%rdf_assert(Obj, niklas:hasObjectMaterialType, niklas:Material_str), rdf_assert(Obj, niklas:hasObjectShapeType, niklas:Shape_str), rdf_assert(Obj, niklas:hasObjectUsedFor, niklas:Affordance_str).
...
check_MaterialType(Obj, MaterialValue):-
	owl_has(Obj, niklas:hasObjectMaterialType, X), owl_individual_of(X, niklas:MaterialValue).
...
check_ShapeType(Obj, ShapeValue):-
	owl_has(Obj, niklas:hasObjectShapeType, X), owl_individual_of(X, niklas:ShapeValue).
...
check_AffordanceType(Obj, AffordanceValue):-
	owl_has(Obj, niklas:hasObjectAffordanceType, X), owl_individual_of(X, niklas:AffordanceValue).
...
comp_newInstance(Instance, 'Cup'):-
	...
	ID='50',
	create_instance_from_class('http://knowrob.org/kb/objects_affordances_attributes.owl#Cup', ID, Instance).
